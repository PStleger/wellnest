...
const Progress = () => {
  ...
  const [QAarray, setQAarray] = useState([{  question: "How are you feeling today?", //0
  answer: ""},
    {
...
{/* {currentQuestionIndex == 4 ? (
  <img src={QAarray[currentQuestionIndex-1].answer} alt="" />
      ...
      currentQuestionIndex == 12 ? 
      (<div>
        <button className="border border-red-200 w-5/6" onClick={handleYes}>Yes</button>
        <button className="border border-red-200 w-5/6" onClick={handleNo}>No</button>
      </div>)
      :null} 
{currentQuestionIndex ==3||currentQuestionIndex==9?(<button onClick={handleCanvas}>Continue</button>):
      <button onClick={handleNextQuestion}>Continue</button>}
    
    {currentQuestionIndex == 14 ?  (<div>
      
      <button onClick = {handleJournal} className="border border-red-200 w-5/6">Yes, Id like to Journal</button>
      <button onClick = {handleNest} className="border border-red-200 w-5/6">Return to my Nest</button>
    </div>):null}
    </div>
  );
};
